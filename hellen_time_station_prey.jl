...
function plot_agent_trajectories(X, Y)
    ...
    ax = Axis(fig[1,1], title="Wolf trajectories", xlabel="x", ylabel="y")

    N = length(X)
    for i in 1:N
        scatter!(ax, X[i], Y[i], label="Wolf $i")
    end
...
display(plot_agent_trajectories(all_wolf_x, all_wolf_y))
