...
all_wolf_x = [Float64[] for _ in 1:5]
all_wolf_y = [Float64[] for _ in 1:5]
...
function animal_step!(wolf, model)
    ...
    if wolf.group == 2              # check agent is a wolf
        ...
        push!(all_wolf_x[i], current_positions[i][1])
        push!(all_wolf_y[i], current_positions[i][2])
    end
end
...
# Plotting the wolf positions over time
function plot_agent_trajectories(X, Y)
    fig = Figure(resolution = (600, 600))
    ax = Axis(fig[1, 1], title = "Agent trajectories", xlabel = "x", ylabel = "y")

    N = length(X)  # number of agents

    for i in 1:N
        lines!(ax, X[i], Y[i], label = "Agent $i")
    end
...
plot_agent_trajectories(all_wolf_x, all_wolf_y)